...
#define LITTLE_FREQ 1224000

TegraNanoProfPolicy::TegraNanoProfPolicy() {
...
TegraNanoProfPolicy::TegraNanoProfPolicy(std::string csvSysFilename, std::string csvApplFilename) : TegraNanoProfPolicy() {
...
void TegraNanoProfPolicy::run(int cycle){
  //update status
...
//  std::cout << "Power consumption: " << sensors->getBigW() << " (big) " << sensors->getLittleW() << " (LITTLE)" << std::endl;        
  ...
  std::map<pid_t, TegraNanoProfPolicy::appl_state>::iterator erase_it = appl_info.begin(); 
  ...
  if(this->csvApplFile){
    *this->csvApplFile << cycle << ";";
    for(std::map<pid_t, TegraNanoProfPolicy::appl_state>::iterator mapIt = appl_info.begin(); mapIt != appl_info.end(); mapIt++) {
