...
#define BIG_FREQ 2035200
#define LITTLE_FREQ 345600

TegraProfPolicy::TegraProfPolicy() {
  ...
  this->sensors->setCPUFreq(TEGRA_LITTLE_CORE_ID, LITTLE_FREQ);
  this->sensors->setCPUFreq(TEGRA_BIG_CORE_ID, BIG_FREQ);
...
TegraProfPolicy::TegraProfPolicy(std::string csvSysFilename, std::string csvApplFilename) : TegraProfPolicy() {
...
void TegraProfPolicy::run(int cycle){
  //update status
...
//  std::cout << "Power consumption: " << sensors->getBigW() << " (big) " << sensors->getLittleW() << " (LITTLE)" << std::endl;        
  ...
  std::map<pid_t, TegraProfPolicy::appl_state>::iterator erase_it = appl_info.begin(); 
  ...
  if(this->csvApplFile){
    *this->csvApplFile << cycle << ";";
    for(std::map<pid_t, TegraProfPolicy::appl_state>::iterator mapIt = appl_info.begin(); mapIt != appl_info.end(); mapIt++) {
