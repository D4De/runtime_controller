...
#define DEFAULT_TIME_WINDOW 1.0
...
typedef struct {
  ...
  bool         useGPU;               // use GPU or not (command from the controller to the application)
} data_t;
...
typedef struct {
  ...
  bool        isMultithreaded;       // specify if the application is multithreaded. In such a case threads have to be forced separately with Cgroups
...
data_t* monitorAttach(char const*, long double, bool, bool);
...
bool useGPU(data_t*);
//get the usleep time
int getUsleepTime(data_t*);
//get the controller pid given its name (usually "main")
pid_t getMonitorPid(const char*);
//sleep for a computed amount of time to satisfy the required throughput (if running faster)
...
double getCurrThroughput(data_t*, double = DEFAULT_TIME_WINDOW);
//get current required throughput
double getReqThroughput(data_t*);
//setter/getter for a specific flag in the application
void setUseGPU(data_t* data, bool); //DO NOTE use monitorPtrRead to get the pointer to the data segment
...
//Return true if the application is multithreaded; otherwise false.
bool isMultithreaded(monitor_t* , pid_t);
