...
data_t* monitorAttach (const char* applName, long double reqThr, bool isMultithreaded, bool hasGpuImpl) {
  //get controller pid
  ...
  monitor->appls[monitor->nAttached].isMultithreaded = isMultithreaded;
  monitor->appls[monitor->nAttached].gpuImpl         = hasGpuImpl;
...
void monitorTick (data_t *data) {
  monitorTick_v(data, 1);
}

void monitorTick_v (data_t *data, int value) {
...
int getUsleepTime(data_t* data) {
  return data->usleepTime;
}
