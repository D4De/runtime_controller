...
//  std::cout << "Power consumption: " << sensors->getBigW() << " (big) " << sensors->getLittleW() << " (LITTLE)" << std::endl;        
  ...
  //remove detached applications info. DO NOTE; do it before updating with new appls 
  ...
  for(int i=0; i < appl_monitor->nAttached; i++){
    ...
    if(appl_info[curr_pid].mapping == MAP_LITTLE_ID) {
      /*on Jetson nano there is no constant step between neighbor frequency levels. so we have to iterate to find the first interesting value...*/
      int k, roundedFreq;
