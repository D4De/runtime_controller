...
/* DO NOTE: this implementation does not support any change in the number of online/offline cores. */

int Utilization::readNumOfCores() {
  //ask to the OS and return the actual number of active cores in the system
  int value;
  FILE* fp;
  fp = popen("cat /proc/stat | grep -c cpu", "r");
  if(fp) {
    fscanf(fp, "%d", &value);
    value--;
    pclose(fp);
    ...
    std::cout << "Error while checking the number of cores" << std::endl;
  }
  return value;
}

Utilization::Utilization() {
  this->numOfCores = Utilization::readNumOfCores();
